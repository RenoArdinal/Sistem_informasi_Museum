...
class M_galeri extends CI_Model{
	...
	function get_galeri_home(){
		$hsl=$this->db->query("SELECT tbl_galeri.*,DATE_FORMAT(galeri_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_galeri ORDER BY galeri_id DESC limit 9");
		return $hsl;
	}
	...
	function simpan_galeri($judul,$album,$user_id,$user_nama,$gambar){
		$this->db->trans_start();
            $this->db->query("insert into tbl_galeri(galeri_judul,galeri_author,galeri_gambar) values ('$judul','$user_nama','$gambar')");
