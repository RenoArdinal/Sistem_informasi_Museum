...
<body class="hold-transition skin-blue sidebar-mini">
                  ...
                  <table id="example1" class="table table-striped" style="font-size:13px;">
                    ...
                    <tbody>
                      ...
                      foreach ($pesan as $pesans) : ?>
                        <tr>
                          <td><?= $no++; ?></td>
                          <td> <a href="#" data-toggle="modal" data-target="#modaldetil<?= $pesans['kode_tiket']; ?>"><?= $pesans['kode_tiket']; ?></a></td>
                          <td><?= $pesans['nama']; ?></td>
                          <td><?= $pesans['email']; ?></td>
                          <td><?= $pesans['tgl']; ?></td>
                          <td><?= $pesans['jam']; ?></td>
                          <td>
                            ...
                            <?php elseif ($pesans['status'] == 0) : ?>
                              <a data-toggle="modal" data-target="#modalkonfirm<?= $pesans['kode_tiket']; ?>" class="btn btn-sm btn-warning text-white">Cek Konfirmasi</a>
                            <?php elseif ($pesans['status'] == 1) : ?>
                              <a data-toggle="modal" href="#" class="btn btn-sm btn-danger">Ditolak</a>
                            <?php elseif ($pesans['status'] == 2) : ?>
                              <a data-toggle="modal" data-target="#modalaktif<?= $pesans['kode_tiket']; ?>" class="btn btn-sm btn-success text-white">Tiket Aktif</a>
  ...
  <?php foreach ($pesan as $pesans) : ?>
    <div class="modal fade" id="modaldetil<?= $pesans['kode_tiket']; ?>" tabindex="-1" role="dialog" aria-labelledby="modalTambah" aria-hidden="true">
          ...
          <div class="modal-body p-4">
            <form action="<?= base_url('tiket/add'); ?>" method="POST">
              <div class="form-group">
                <h1><b>
                    <center><?= $pesans['kode_tiket']; ?></center>
                  </b></h1>
                <hr>
                <NO for="no_ktp">ID Tiket</label>
                  <input type="text" readonly="" class="form-control" required="" name="no_ktp" value="<?= $pesans['kode_tiket']; ?>" id="no_ktp" placeholder="NIK">
              ...
              <div class="form-group" required="">
                ...
                <div class="custom-control custom-radio custom-control-inline">
                  <input type="radio" readonly="" id="jenis_kemain1" name="jk" value="laki-laki" <?php if ($pesans['jenis_kelamin'] == 'laki-laki') {
                                                                                                    echo 'checked';
                ...
                <div class="custom-control custom-radio custom-control-inline">
                  <input type="radio" readonly="" id="jenis_kemain2" name="jk" value="perempuan" <?php if ($pesans['jenis_kelamin'] == 'perempuan') {
                                                                                                    echo 'checked';
                                                                                                  } ?> class="custom-control-input">
  ...
  <!-- MODAL KONFIRMASI  -->
  <?php foreach ($pesan as $pesans) : ?>
    <div class="modal fade" id="modalkonfirm<?= $pesans['kode_tiket']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
          ...
          <div class="modal-body">
            ...
            <div class="row" style="margin-bottom:30px; margin:4px auto;">
              ...
              <form action="<?= base_url('admin/tiket/update'); ?>" method="POST" class="mb-5">
                <h1><b>
                    <center><img width="300px" src="<?= base_url('assets/img/bukti_tf/') . $pesans['foto']; ?>" alt="foto" /></center>
                  </b></h1>
                <hr>

                <div class="form-group">
                  <NO for="no_ktp">ID Tiket</label>
                    <input type="text" name="kode_tiket" id="kode_tiket" value="<?= $pesans['kode_tiket']; ?>" readonly="" class="form-control" required="" placeholder="NIK">
                    <small class="form-text form-danger"><?= form_error('npm'); ?></small>
                </div>
            ...
            <form action="<?= base_url('admin/tiket/update'); ?>" method="POST" class="mt-5">
              <input type="hidden" name="kode_tiket" id="kode_tiket" value="<?= $pesans['kode_tiket']; ?>" readonly="" class="form-control" required="" placeholder="NIK">
  ...
  <?php foreach ($pesan as $ps) : ?>
    <div class="modal fade" id="modalaktif<?= $ps['kode_tiket']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
          ...
          <div class="modal-body">
            <h3>Data tiket dengan id <b><?= $ps['kode_tiket']; ?></b> akan digunakan ?</h3>
            <hr>
            <div class="row" style="margin-bottom:30px; margin:4px auto;">
              <form action="<?= base_url('admin/tiket/update'); ?>" method="POST" class="mt-5">
                <input type="hidden" name="kode_tiket" id="kode_tiket" value="<?= $ps['kode_tiket']; ?>" readonly="" class="form-control" required="" placeholder="NIK">
